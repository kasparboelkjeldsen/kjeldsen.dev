  ...
  >
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="relative">
          <select
            ...
            class="bg-gray-800 border border-gray-600 text-white text-sm rounded px-3 py-1.5 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none min-w-[200px]"
          >
        ...
        <button
          ...
          @click="impersonate"
          class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold px-3 py-2 rounded transition-colors uppercase tracking-wide"
        >
          Impersonate
        </button>
    ...
    <div class="flex items-center gap-6 text-xs text-gray-400">
        ...
        <div v-for="group in scores" :key="group.id" class="flex flex-col gap-1">
          ...
          <div class="flex gap-2">
            <div
              ...
              :class="{ 'ring-1 ring-amber-500': persona.score > 0 }"
            >
              <span class="text-gray-300">{{ persona.title }}</span>
              <span
                class="font-mono font-bold"
                :class="persona.score > 0 ? 'text-amber-500' : 'text-gray-600'"
              >
                {{ persona.score }}
              </span>
            </div>
      ...
      <div v-if="activeCookie">
        Active Override:
        <span class="text-amber-500 font-bold">{{ activeCookie }}</span>
      </div>
