<template>
  <div :class="itemClasses">
    <!-- Render the content -->
    <div class="mb-2">
      <BlocksResolverComponent
        v-if="item.content"
        :data="item.content"
        :columns="item.columnSpan ?? 12"
      />
    </div>

    <!-- Areas -->
    <div v-if="item.areas && item.areas.length">
      <div v-for="(area, areaIndex) in item.areas" :key="areaIndex" class="">
        <div class="mb-2 font-bold">Area: {{ area.alias }}</div>
        <div class="grid gap-2" :class="`grid-cols-1 sm:grid-cols-${area.columnSpan}`">
          <GridItem
